window.startGrammarGame = function(questionsFromApi) {
    // ここでグローバル変数に入れておく
    window.grammarQuestions = questionsFromApi;
...
class MainScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image('q1_image', '../assets/correct_images/q1_image.png');
        this.load.image('q2_image', '../assets/correct_images/q2_image.png');
        this.load.image('q3_image', '../assets/correct_images/q3_image.png');
        this.load.image('q4_image', '../assets/correct_images/q4_image.png');
        this.load.image('q5_image', '../assets/correct_images/q5_image.png');

        this.load.image('q1_wrong_image', '../assets/wrong_images/q1_image.png');
    }

    create() {     
        
        const apiQuestions = window.grammarQuestions || [];

        let sentence = "No data";

        if (apiQuestions.length > 0) {
            sentence = apiQuestions[0].sentence;  
        ...
        // noun:0, verb:1, adj:2, other:3
        const colors = [ 0x6155F5, 0x0088FF, 0x91FC52, 0x00C8B3];

        this.questions = [
            {
                japanese: [
                    { word: "さる", color: colors[0] },
                    { word: "が", color: colors[3] },
                    { word: "バナナ", color: colors[0] },
                    { word: "を", color: colors[3] },
                    { word: "食べます", color: colors[1] }
                ],
                wrong_answers: [
                    {
                        words: ["バナナ", "が", "さる", "を", "食べます"], 
                        image_key: "q1_wrong_image"
                    }                                
                ],
                english: "A monky eats a banana.",
                image_key: "q1_image",
                wrong_image_key: "q1_image"                            
            },
            {
                japanese: [
                    { word: "私", color: colors[0] },
                    { word: "は", color: colors[3] },
                    { word: "パン", color: colors[0] },
                    { word: "を", color: colors[3] },
                    { word: "食べます", color: colors[1] }
                ],
                wrong_answers: [                                                               
            ...
            },
            {
                japanese: [
                    { word: "私", color: colors[0] },
                    { word: "は", color: colors[3] },
                    { word: "日本語", color: colors[0] },
                    { word: "を", color: colors[3] },
                    { word: "勉強します", color: colors[1] }
                ],
                wrong_answers: [                                                               
                ],
            ...
            {
                japanese: [
                    { word: "彼", color: colors[0] },
                    { word: "は", color: colors[3] },
                    { word: "映画", color: colors[0] },
                    { word: "を", color: colors[3] },
                    { word: "見ます", color: colors[1] }
                ],
                wrong_answers: [                                                               
                ],
                english: "He watches a movie.",
                image_key: "q4_image"
            },
            {
                japanese: [
                    ...
                    { word: "は", color: colors[3]},
                    { word: "彼", color: colors[0]},
                    { word: "の", color: colors[3]},
                    { word: "友人", color: colors[0]},
                    { word: "と", color: colors[3]},
                    { word: "映画", color: colors[0]},
                    { word: "を", color: colors[3]},
                    { word: "見ませんでした", color: colors[1]}
                ],
                wrong_answers: [                                                               
                ],
        ...
        // // 画面幅と高さから使える最大サイズを求める
        // const maxW = this.cx / 10; 
        // const maxH = this.cy / 10; 
        // // 正方形なので小さい方を採用
        // const size = Math.min(maxW, maxH);
        // // 最初の問題の画像を表示
        // this.questionImage = this.add.image(this.whole_W/3, this.whole_H/6, this.questions[this.cur_question_i].image_key)
        //     .setOrigin(0.5, 0.5)
        //     .setDisplaySize(size, size);
            
        // 画面幅と高さから使える最大サイズを求める（画像を入れる枠のサイズ）
        const maxW = this.cx / 2;   // 好きな大きさに調整
        ...
        this.questionImage = this.add.image(
            this.whole_W / 3,
            this.whole_H / 6,
            this.questions[this.cur_question_i].image_key  
        ).setOrigin(0.5);
        ...
        // 枠(maxSize × maxSize)に収まるような scale を計算（比率はそのまま）
        ...
        this.bin_body_R = this.whole_W / 30;                    

        // 瓶と蓋の線の太さ
        this.bin_lid_line_weight = 2;
    ...
    check_sentence() {
        ...
        // 間違えの数字を作成
        const wrong_sentence_nums = cur_q.wrong_answers.map(wrong_arr =>
            wrong_arr.words.map(word => word_to_index[word]).join("")
        );
        
        const hit_index = wrong_sentence_nums.findIndex(
            w => w === user_sentence_num
        );
        
        console.log('hit_index ', hit_index );
        console.log('cur_q', cur_q);
        console.log('correct_sentence_num:', correct_sentence_num);
        console.log('user_sentence_num:', user_sentence_num);
        ...
        if (correct_sentence_num === user_sentence_num) {                        
            ...
            const wrong_img_key = cur_q.wrong_answers[hit_index].image_key;
            
            // 間違いの画像を表示
            this.questionImage.setTexture(wrong_img_key);                        
    ...
    make_bin_body_W() {
        ...
        this.widths = cur_q_words.map(item => {
            ...
            const paddingX = this.whole_W / 20;
            const bin_body_W = Math.max(120, tmp.width + paddingX);
    ...
    small_pipe(container) {                    
        ...
        second_rect.setStrokeStyle(2, 0x000000);                    
